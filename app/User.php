...
class User extends Authenticatable
{
    ...
    public function dont_want($itemId)
    {
        ...
        if ($exist) {
            ...
            $deleteSql = '';
            $deleteSql = $deleteSql . "DELETE FROM item_user       ";
            $deleteSql = $deleteSql . "      WHERE user_id = ?     ";
            $deleteSql = $deleteSql . "        AND item_id = ?     ";
            $deleteSql = $deleteSql . "        AND type    = 'want'";

            \DB::delete($deleteSql, [\Auth::user()->id, $itemId]);
    ...
    public function dont_have($itemId)
    {
        ...
        if ($exist) {
            ...
            $deleteSql = $deleteSql . "      WHERE user_id = ?     ";
            $deleteSql = $deleteSql . "        AND item_id = ?     ";
            $deleteSql = $deleteSql . "        AND type    = 'have'";
